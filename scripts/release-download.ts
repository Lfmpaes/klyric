...
async function download(url: string, destination: string): Promise<void> {
  const response = await fetch(url, {
    headers: { Accept: "application/octet-stream", "User-Agent": "klyric-installer" },
    redirect: "follow",
  });
...
export async function downloadLatestRelease(
  ...
  const response = await fetch(`${apiBase}/repos/${repository}/releases/latest`, {
    headers: { Accept: "application/vnd.github+json", "User-Agent": "klyric-installer" },
  });
  if (!response.ok) {
    throw new Error(`Could not resolve the latest KLyric release (${response.status}).`);
  }
  ...
  if (!archive || !checksums) {
    throw new Error("The latest KLyric release is missing its archive or checksums.");
  }
  ...
  const verification = Bun.spawn(["sha256sum", "--check", basename(checksumPath)], {
    cwd: destination,
    stderr: "inherit",
    stdout: "inherit",
  });
  ...
  if ((await extraction.exited) !== 0) throw new Error("Could not extract KLyric release.");
