...
import { installLocal, parseUninstallOptions, uninstallLocal } from "./local-installation";
import { downloadLatestRelease } from "./release-download";

const VERSION = (await Bun.file(join(import.meta.dir, "../package.json")).json())
  .version as string;
...
else if (command === "uninstall") await uninstallLocal(parseUninstallOptions(arguments_));
else if (command === "version") console.info(VERSION);
else if (command === "help" || command === "--help" || command === "-h") console.info(usage());
