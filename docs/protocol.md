...
Cider plugin, loopback bridge, and Plasma widget. It is platform-neutral: its
runtime code uses only standard JavaScript APIs available in browsers and Bun.
The package exposes its normal API at `@klyric/protocol` and sanitized fixtures
at `@klyric/protocol/fixtures`.
...
- The plugin increments `sequence` for every publication within one
  `sessionId`. A bridge rejects duplicate or regressing sequences within that
  session; a new session may restart at sequence zero.

## State publication

Every `KLyricState` includes `protocolVersion`, `sequence`, `sessionId`, and
an ISO-8601 `emittedAt` timestamp, plus playback metadata, lyric availability,
the source kind, and optional adjacent lines. `track` is either `null` or a
flat object with optional identity and display fields. Lyric lines are either
`null` or a flat object with text, optional timing/index data, and an optional
instrumental marker.
...
`parseKLyricState`, `parseServerMessage`, and `parseClientMessage` reject
invalid cross-process data with `ProtocolValidationError`. The first two accept
optional `StateValidationOptions` so the bridge can provide its clock when
checking a state timestamp.

- Serialized input is limited to 64 KiB.
- Lyric text is trimmed (internal whitespace is preserved), and a blank lyric
  line becomes `null`. Lines are limited to 2,000 Unicode code points.
- Track title, artist, album, and identifier fields are limited to 500 code
  points; artwork URLs are limited to 2,048.
- Millisecond values and sequence values must be non-negative safe integers.
  Lyric end time cannot precede start time.
...
- `emittedAt` must parse as a timestamp within five minutes of the bridge clock
  by default. Tests and bridge code can supply a different clock/skew through
